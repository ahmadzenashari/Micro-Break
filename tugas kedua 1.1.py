...
# === Pilihan Batas Tanpa Wajah (detik) ===
BATAS_TANPA_WAJAH = {
    "5 detik": 5,
    "10 detik": 10,
    "15 detik": 15,
...
def pilih_pengaturan_gui():
    """GUI untuk memilih resolusi, durasi maksimal, dan batas tanpa wajah."""
    root = tk.Tk()
    ...
    # === Batas meninggalkan tempat ===
    tk.Label(root, text="Batas Tanpa Wajah:", font=("Arial", 11)).pack(pady=5)
    selected_batas = tk.StringVar(value="15 detik")
    combo_batas = ttk.Combobox(root, textvariable=selected_batas,
                               values=list(BATAS_TANPA_WAJAH.keys()), state="readonly")
    combo_batas.pack(pady=5)
    ...
    if start_pressed.get():
        return selected_res.get(), selected_durasi.get(), selected_batas.get()
    else:
        return None, None, None


def jalankan_deteksi(res_key, durasi_key, batas_key):
    ...
    batas_tidak_terdeteksi = BATAS_TANPA_WAJAH[batas_key]
    ...
    print(f"[INFO] Batas tanpa wajah: {batas_key}")
    ...
    while True:
        ...
        # === Jika tidak ada wajah ===
            ...
            # jika tidak mendeteksi wajah lebih dari batas yang dipilih
        ...
        cv2.putText(frame, f"Tanpa wajah: {int(waktu_tanpa_wajah)}s (batas {batas_key})",
                    (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 100, 255), 2)
...
def main():
    while True:
        res_key, durasi_key, batas_key = pilih_pengaturan_gui()
        if res_key is None or durasi_key is None or batas_key is None:
            print("Dibatalkan oleh pengguna.")
            break

        hasil = jalankan_deteksi(res_key, durasi_key, batas_key)
